...
или клавиатуру. Заряд берётся из двух источников, которые ничего не будят:
данных Windows по Bluetooth и G HUB, пока он запущен (он опрашивает свои
устройства в любом случае). У беспроводных устройств без Bluetooth и без
G HUB заряд не показывается. Пока окно спрятано в трей и виджет выключен,
